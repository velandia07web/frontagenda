...
import "./PrecioPorZonaPacks.css";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faBoxes } from "@fortawesome/free-solid-svg-icons";
import { getZoneWithPacks, Zone } from "../../servicios/zone";

const TablaPreciosPorZonaPacks: React.FC = () => {
  ...
  useEffect(() => {
    const fetchZones = async () => {
      try {
        const zoneData = await getZoneWithPacks();
        setZones(zoneData);
      } catch (error) {
  ...
  return (
    <div className="homepage-container">
      ...
      <div className="d-flex">
        ...
        >
          <h1 className="text-center titulo-tabla">Zonas parametrizadas con paquetes</h1>
          <div className="dashboard-cardsPrices" >
            {Array.isArray(zones) && zones.length > 0 ? (
              zones.map((zone) => (
                <div key={zone.id} className="card" onClick={() => navigate(`/tablaPreciosPacks/${zone.id}`)}>
...
export default TablaPreciosPorZonaPacks;
